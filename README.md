...
При `capture-part-output` (Части 2–9) валидатор дополнительно проверяет каждую карточку документа против реально загруженной страницы `URL2` — не только «жива ли ссылка», но и «содержит ли страница именно заявленный `Структурный элемент`» (content-support, BLOCKER) и «не противоречит ли странице заявленный `Статус: действует`» (freshness, BLOCKER). Обе проверки полностью детерминированы, без LLM-судьи. См. `AGENTS.md`, «Правила Ссылок И Верификации».
...
Первая команда прогоняет golden-набор (26 тестов) на content-support/freshness/структурную валидацию Части 3 — без реальных сетевых вызовов. Вторая — детерминированный отчёт, честно показывающий, какие acceptance-gates применимы к этому слою и выполнены, а какие (liveness/provenance/бюджет) вне его объёма и закрыты другими механизмами (Defense 1–3, `check_url2_title_audit_at_capture`).
