...
## Обновление 30.08.2026 — найден корень WinError 10013

Расследование (прямое воспроизведение того же HTTP-запроса разными способами) показало:
`WinError 10013` — не системная блокировка `python.exe`/`urllib` как таковых, а блокировка
сокета внутри **sandbox-режима исполнения самого coding-агента**. Тот же самый запрос,
выполненный **вне sandbox**, прошёл успешно (`SUCCESS status: 200`). Это совпадает с ранее
отмеченными всплывающими окнами запроса сетевого доступа (см. `AGENTS.md`, блокер
«требуется внешнее разрешение на действие вне sandbox») — они и есть штатный механизм
получить сетевой доступ, просто их обходили вместо того, чтобы одобрять.

**Следствие:** `agent_supplied=true` записи **больше не исключены** из Defense 1
(независимая перепроверка URL) и Defense 3 (анализ временных меток) в `notary_agent.py` —
обе теперь проверяют такие записи наравне со всеми. Режим `--title --preview` остаётся
как быстрый путь записи (не требует ждать ответа сервера в момент захвата), но независимая
проверка по нему выполняется отдельно, как и по всем остальным записям.

**ТРЕБОВАНИЕ:** если при `fetch-and-log` в legacy-режиме (без `--title`) или при прогоне
Defense 1 всплывает запрос на сетевой доступ вне sandbox — одобрять его, не обходить.
Обход через `agent_supplied` — резервный путь на случай, если разрешение получить
нельзя (например, безлюдный batch-прогон), а не способ по умолчанию избежать одобрения.
